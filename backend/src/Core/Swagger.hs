...
import Data.OpenApi
  (Definitions, HasType(type_), NamedSchema(NamedSchema),
  OpenApiType(OpenApiObject), Referenced, Schema, ToSchema, declareSchemaRef,
  properties, required)
...
-- @
namedSchema
  :: forall t f . (Typeable t, Applicative f)
  => (Schema -> Schema)
  -> f NamedSchema
...
-- only single field.
declareSingleFieldSchema
  :: forall field a proxy . (ToSchema field, Typeable a)
  => Text
  -> proxy a
...
declareSingleFieldSchema fieldName _ = do
  ...
  namedSchema @a $ \s -> s
    & properties .~ fromList [(fieldName, fieldsSchema)]
    & required .~ [fieldName]
